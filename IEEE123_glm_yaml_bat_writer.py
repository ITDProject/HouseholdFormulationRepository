...
WH = '_WH' + str(WHP*100) + '%'
housetype = [["Small_Poor_Poor", "Small_Normal", "Small_Good"], ["Normal_Poor", "Normal_Normal_Normal",  "Normal_Good"], ["Large_Poor", "Large_Normal", "Large_Good_Good"]]
feeder = 'feeder123_'
glm = '.glm'
...
for objname,objdata in objects.items():
	objects[objname]['house_mix'] = {j : [[H_index-1, TI_index-1] for i in range(v)] for j,v in objects[objname]['no_houses'].items()}

feeder_json = 'runHouses_' + str(TotalHouses) + '.bat'
...
for i in range(NDsystems):
	if mix != 1:
		feeder_glm = feeder + housetype[H_index-1][TI_index-1] + TH + str(i+1) + glm
	else:
		feeder_glm = feeder + 'mix' + TH + glm
...
with open( AgentType + '.yaml', 'w') as outfile: 
